...
const GlassButton: React.FC<GlassButtonProps> = ({ href, children, className = '', fullWidth, trackLabel }) => (
  <Glass
    variant="strong"
    rounded="rounded-lg"
    className={`relative px-8 py-4 glow-button text-center ${fullWidth ? 'w-full sm:w-auto' : ''} ${className}`}
  >
    <a
      href={href}
      className="absolute inset-0 flex items-center justify-center gap-2.5 text-[15px] font-semibold text-[#f5f0eb] tracking-wide"
      target="_blank"
          ...
          {features.map((feature, i) => (
            ...
            >
              <div className={`flex-1 ${i === 1 ? 'lg:order-1' : ''}`}>
                ...
                <Glass variant="strong" rounded="rounded-lg" className="relative px-6 py-2.5 inline-block glow-button">
                  <button
                    className="absolute inset-0 flex items-center justify-center gap-2 text-sm font-medium text-[#f5f0eb]"
